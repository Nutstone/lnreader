...
const TTSController = () => {
  ...
  const endDrag = e => {
    ...
    if (moved && reader.generalSettings.val.AudiobookEnable) {
      // In audiobook mode, start full chapter playback (no element targeting)
      if (!audiobook.started) {
        audiobook.start();
        const playPauseButton = document.getElementById('TTS-PlayPause');
        if (playPauseButton) playPauseButton.innerHTML = pauseIcon;
      }
    } else if (moved && hoverElement && reader.generalSettings.val.TTSEnable) {
  ...
  return div(
    {
      ...
      class: () =>
        [
          ...
          reader.generalSettings.val.AudiobookEnable
            ? ''
            : 'hidden',
