...
const readJSON = async <T,>(path: string): Promise<T | null> => {
  try {
    ...
    return JSON.parse(await NativeFile.readFile(path)) as T;
  } catch {
    return null;
  }
};
...
const VoiceCastScreen = ({ navigation, route }: VoiceCastScreenProps) => {
  ...
  useEffect(() => {
    let cancelled = false;
    (async () => {
      const loadedGlossary = await readJSON<CharacterGlossary>(
      ...
      const map = await readJSON<VoiceMap>(`${novelDir}/voice-map.json`);
      if (cancelled) {
        return;
      }
