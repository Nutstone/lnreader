...
const dirSize = async (path: string): Promise<number> => {
  ...
  try {
    ...
    while (stack.length) {
      const dir = stack.pop()!;
      for (const entry of await NativeFile.readDir(dir)) {
        ...
        <List.Section>
          ...
          >
            <View style={styles.switchLabel}>
              ...
              >
                &lsquo;Prepare audiobook&rsquo; also synthesizes the
                chapters&apos; audio so playback starts instantly. Takes
                minutes per chapter and uses ~10 MB each.
              </Text>
            </View>
            <Switch
